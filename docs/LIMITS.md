...
are not counted as caps, and 218 + 7 is the 225 constants this generator parses out of `src/`.
...
The `class` column below carries that answer where it is known. **120 of 218 caps are classified
(39 INDEXING, 41 OUTPUT, 40 BOUNDARY); the remaining 98 render `—`, which means NOT YET
...
One table for each of the 86 files that declare a cap — the 218 caps counted above, and no parameter.
...
| `kSliceCallerStackDepth` | `256` | BOUNDARY | — |
